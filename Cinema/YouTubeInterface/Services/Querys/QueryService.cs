...
namespace YoutubeDownloader.Services
{
    public class QueryService
    {
        ...
        public IReadOnlyList<Query> ParseMultilineQuery(string query) =>
            query.Split(new string[] { Environment.NewLine }, StringSplitOptions.None).Select(ParseQuery).ToArray();
        ...
        public async Task<ExecutedQuery> ExecuteQueryAsync(Query query)
        {
            ...
            {
                var videos = await _youtube.Search.GetVideosAsync(query.Value).CollectAsync(9);
